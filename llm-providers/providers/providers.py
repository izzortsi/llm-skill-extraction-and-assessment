...
def create_provider(
    ...
    if provider_name == "claude-code":
        ...
        # Direct file import to avoid tools namespace collision with other pipelines
        candidate = Path(__file__).resolve().parent
        adapter_path = None
        for _i in range(6):
            candidate = candidate.parent
            path = candidate / "cli" / "tools" / "claude_code_provider.py"
            if path.is_file():
                adapter_path = path
                break
        if adapter_path is None:
            raise RuntimeError("cannot locate cli/tools/claude_code_provider.py")
