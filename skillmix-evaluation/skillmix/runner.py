...
Usage:
    python -m skillmix.runner --config experiment.json
    python -m skillmix.runner --tasks tasks.json --skills-dir skills/ --models qwen3:0.6b,llama3.2:1b
"""
...
class _OpenAIProvider:
    ...
    interface expected by skillmix.harness (`.chat(messages)` returning an
...
def _compute_summary(episodes: List[Dict]) -> Dict[str, Any]:
    ...
    Delegates to analytics.summary.compute_summary.
...
def main() -> None:
    ...
    Delegates to cli.run_skillmix for argument parsing and execution.
    Prefer using ``python -m cli.main run-skillmix`` instead.
    """
    from cli.run_skillmix import run
