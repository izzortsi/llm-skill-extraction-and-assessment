"""c1_tools -- loaders and text extraction utilities."""

__all__: list[str] = []

try:
    from c1_tools.skill_loader import SkillLoader
    from c1_tools.task_loader import TaskLoader, BenchTask, load_verification_tasks
    __all__ += ["SkillLoader", "TaskLoader", "BenchTask", "load_verification_tasks"]
except ImportError:
    pass

try:
    from c1_tools.text_extractor import process_file, process_directory
    __all__ += ["process_file", "process_directory"]
except ImportError:
    pass

# Backwards compatibility: re-export data structures and text utils that
# were previously defined directly in c1_tools.text_extractor.
try:
